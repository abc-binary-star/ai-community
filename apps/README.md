...
面向社区用户的一站式实用工具集合。每个工具是**自包含的子服务**（独立 Go Module + 自带 Web 界面），按 Commons 设计语言构建，通过统一入口接入社区前端（L0 卡片交互层）。

| 子服务 | 说明 | 技术栈 | 状态 |
|--------|------|--------|------|
| [epub-translator](./epub-translator/) | EPUB 书籍翻译：上传外文 EPUB，AI Agent 分章解析、保持版式，输出简体中文版 | Go + CloudWeGo Hertz + Eino + 豆包 ARK | ✅ 已迁移（首个子服务） |
