...
if [ ! -f .env.production ]; then
    echo "⚠️  .env.production 不存在，从模板复制..."
    cp .env.production .env.production 2>/dev/null || {
        echo "❌ 请先创建 .env.production 文件"; exit 1
    }
fi
...
if [[ "$JWT_SECRET" == *"change-me"* ]]; then
    ...
    echo "   生成命令: node -e \"console.log(require('crypto').randomBytes(32).toString('hex'))\""
    echo ""
...
echo "[5/5] 等待数据库就绪并迁移..."
sleep 3
docker compose -f docker-compose.prod.yml exec -T server npx prisma migrate deploy 2>/dev/null || \
